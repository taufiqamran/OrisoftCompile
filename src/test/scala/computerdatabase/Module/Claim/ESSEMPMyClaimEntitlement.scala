...
object ESSEMPMyClaimEntitlement {
  ...
  val uri3 = "https://fonts.googleapis.com/css"
    ...
    def loadApplication = {
      exec(http("Load My Claim Entitlement Listing Page")
        .get("/Claim/EntitlementListing")
        .check(regex("""<input name="__RequestVerificationToken" type="hidden" value="(.+?)" />""").find.saveAs("__RequestVerificationTokenMCEL"))
        .check(css("input[name='Year']", "value").saveAs("textYear"))


        .check(bodyString.saveAs("MCELBody"))
      )
        ...
        .exec(http("request_77")
          .get("/signalr/negotiate?clientProtocol=2.1&userid=KM00562&connectionData=%5B%7B%22name%22%3A%22notificationhub%22%7D%5D&_=1620104689650")
        )
        .pause(445.milliseconds)
        .exec(http("request_78")
          .get("/signalr/start?transport=webSockets&clientProtocol=2.1&userid=KM00562&connectionToken=%2BreD0ZMO3LN33XVwreetflpG5wrX3aECs0U%2BrdhuxNRP81y5Pzx9B0ITOlOWWtuPD3hC7aYYJ%2Bx3JldZKCPQBvwjKfA0D267BTzPvASG5wE3vWUL3uOz6URAidq7ocTIfFZPCcmP2AyoEIAVh3OcMjtqiMj9dG97%2BWWhCAV9tTM%3D&connectionData=%5B%7B%22name%22%3A%22notificationhub%22%7D%5D&_=1620104689651")
        )
        .pause(4)
        .exec(http("request_79")
          .get(uri1 + "/AuthenticationService.Authenticate?1shttps%3A%2F%2Forisoftst.orisoftsaas.com%2FClaim%2FEntitlementListing&4sAIzaSyDu3r6sgbYDPJTjUH9Dun7RSWAyB5aQi8A&callback=_xdc_._bro8ga&key=AIzaSyDu3r6sgbYDPJTjUH9Dun7RSWAyB5aQi8A&token=113466")
        )
        .pause(6)
    ...
    def searchEntitle = {
      exec(http("Search Claim Entitlment by Year")
        .post("/Claim/ClaimEntitlementListingGridViewPartial")

        .formParam("CustomSearchParameters[0][Name]", "Year")
        .formParam("CustomSearchParameters[0][Value]", "${textYear}")
        .check(bodyString.saveAs("MCELBody2"))
      )
    ...
    def logout = {
      exec(http("Submit LogOff")
        ...
        .formParam("__RequestVerificationToken", "${__RequestVerificationTokenMCEL}")
